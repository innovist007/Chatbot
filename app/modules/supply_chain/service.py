...
from app.config import ANALYTICS_TTL, CACHE_V, Settings, make_cache_key

logger = logging.getLogger(__name__)

TABLE         = "`innovist-master-data.shopify.rto_analytics`"
PRODUCT_TABLE = "`innovist-master-data.shopify.v_product_rto_analytics`"
...
class SupplyChainService:
    ...
    def _get_cache(self, key: str) -> Any | None:
        ...
        try:
            ...
            if raw:
                logger.info("🎯 Cache HIT  [sc]: %s", key)
                return json.loads(raw)
            logger.info("❌ Cache MISS [sc]: %s", key)
            return None
        except Exception as e:
    ...
    def _set_cache(self, key: str, value: Any, ttl: int = ANALYTICS_TTL) -> None:
        ...
        try:
            ...
            logger.info("💾 Cache SET  [sc]: %s (ttl=%ss)", key, ttl)
        except Exception as e:
    ...
    def _where(self, f: SupplyChainFilters) -> tuple[str, list]:
        ...
        return "DATE(created_at_in_timezone) BETWEEN @start_date AND @end_date", params
    ...
    def _segment_table(
        ...
        SELECT
          {segment_column} AS segment,
          DATE(created_at_in_timezone) AS d,
          SAFE_DIVIDE(COUNTIF(shipment_status IN {rto_in}), COUNT(name)) AS rto_pct
        FROM {self.table}
        WHERE DATE(created_at_in_timezone) BETWEEN
    ...
    def trend(
        ...
        query_table = PRODUCT_TABLE if segment == "product" else self.table

        if granularity == "DoD":
            bucket_expr = "FORMAT_DATE('%Y-%m-%d', DATE(created_at_in_timezone))"
        elif granularity == "WoW":
            bucket_expr = "FORMAT_DATE('%G-W%V', DATE(created_at_in_timezone))"
        else:
            bucket_expr = "FORMAT_DATE('%Y-%m', DATE(created_at_in_timezone))"
        ...
        if seg_col:
            ...
            if sub_filter and sub_filter.lower() != "all":
                values = [v.strip() for v in sub_filter.split(",") if v.strip()]
                if len(values) == 1:
                    where += f" AND {seg_col} = @sub_filter"
                    params.append(bigquery.ScalarQueryParameter("sub_filter", "STRING", values[0]))
                elif len(values) > 1:
                    where += f" AND {seg_col} IN UNNEST(@sub_filters)"
                    params.append(bigquery.ArrayQueryParameter("sub_filters", "STRING", values))
    ...
    def segment_options(self, segment: str) -> list[str]:
        ...
        if not col:
            return []
        cache_key = f"sc_segment_options:{CACHE_V}:{segment}"
        cached = self._get_cache(cache_key)
        if cached:
            return cached
        tbl = PRODUCT_TABLE if segment == "product" else self.table
        sql = f"""
    ...
    def all_segment_options(self) -> dict[str, list[str]]:
        # exclude product — too many values, loaded on-demand via /segment-options?segment=product
        return {seg: self.segment_options(seg) for seg in SEGMENT_COLUMNS if seg != "product"}

    # ---------- latest data date (for AI summary) ----------
    def get_latest_date(self) -> date | None:
        sql = f"SELECT MAX(DATE(created_at_in_timezone)) AS d FROM {self.table}"
        rows = self._run(sql, [])
