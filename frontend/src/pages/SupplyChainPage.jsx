...
function rtoPillTone(rto) {
  if (rto == null) return "sc-gray";
  if (rto <= 0.10) return "sc-green";
  if (rto <= 0.15) return "sc-amber";
  return "sc-red";
}

function etaPillTone(eta) {
  if (eta == null) return "sc-gray";
  if (eta >= 0.75) return "sc-green";
  if (eta >= 0.70) return "sc-amber";
  return "sc-red";
}

function ndrPillTone(ndr) {
  if (ndr == null) return "sc-gray";
  if (ndr <= 0.15) return "sc-green";
  if (ndr <= 0.20) return "sc-amber";
  return "sc-red";
}

function MoMDelta({ pp }) {
  if (pp == null) return <span className="text-sc-gray-400">—</span>;
  const v = pp * 100;
  // RTO is lower-is-better: negative delta = improvement (green), positive = worse (red)
  const isImprovement = v < 0;
  const isFlat = Math.abs(v) < 0.05;
  const arrow = isFlat ? "▬" : isImprovement ? "▼" : "▲";
  const tone = isFlat ? "text-sc-gray-600" : isImprovement ? "text-sc-green" : "text-sc-red";
  return (
...
function RtoTrend7d({ points }) {
  if (!points || points.length === 0) {
    return <span className="text-sc-gray-400">—</span>;
  }
  ...
  const stroke = isImproving ? "#3B6D11" : "#A32D2D";
  return (
...
const WAREHOUSE_COLUMNS = [
  { key: "segment", label: "Warehouse", render: (r) => <span className="font-medium text-sc-gray-900">{r.segment}</span> },
  { key: "orders", label: "Orders", align: "right", mono: true, render: (r) => fmt.num(r.orders) },
  ...
  { key: "cpo", label: "CPO", align: "right", mono: true, render: () => <span className="text-sc-gray-400">—</span> },
  ...
  { key: "target", label: "Target", align: "center", render: () => <span className="text-sc-gray-400">—</span> },
...
const PAYMENT_COLUMNS = (totalOrders) => [
  { key: "segment", label: "Payment", render: (r) => <span className="font-medium text-sc-gray-900">{r.segment}</span> },
  { key: "orders", label: "Orders", align: "right", mono: true, render: (r) => fmt.num(r.orders) },
  ...
  { key: "first_attempt_pct", label: "1st att del %", align: "right", mono: true, render: () => <span className="text-sc-gray-400">—</span> },
];

function VolShare({ orders, total }) {
  if (!total || total <= 0) return <span className="text-sc-gray-400">—</span>;
  const pct = (orders / total) * 100;
  const barW = Math.max(Math.min(pct, 100), 1);
  return (
    <div className="flex items-center gap-2 justify-center">
      <div className="w-16 h-2 bg-sc-gray-200 rounded overflow-hidden">
        <div className="h-2 bg-sc-purple rounded" style={{ width: `${barW}%` }} />
      </div>
      <span className="text-[10px] font-semibold text-sc-purple tabular-nums">{pct.toFixed(1)}%</span>
    </div>
  );
}

const COURIER_COLUMNS = (totalOrders) => [
  { key: "segment", label: "Courier", render: (r) => <span className="font-medium text-sc-gray-900">{r.segment}</span> },
  { key: "orders", label: "Orders", align: "right", mono: true, render: (r) => fmt.num(r.orders) },
  { key: "rto_pct", label: "RTO %", align: "center", render: (r) => <Pill tone={rtoPillTone(r.rto_pct)}>{fmt.pct(r.rto_pct)}</Pill> },
  { key: "mom_delta", label: "MoM Δ", align: "center", render: (r) => <MoMDelta pp={r.rto_delta_pp} /> },
  { key: "cod_tat", label: "COD TAT", align: "right", mono: true, render: () => <span className="text-sc-gray-400">—</span> },
  { key: "cod_sla", label: "SLA?", align: "center", render: () => <span className="text-sc-gray-400">—</span> },
  { key: "rto_tat_days", label: "RTO TAT", align: "right", mono: true, render: (r) => `${r.rto_tat_days.toFixed(1)}d` },
  { key: "rto_sla", label: "SLA?", align: "center", render: () => <span className="text-sc-gray-400">—</span> },
  { key: "pre_tat", label: "Pre TAT", align: "right", mono: true, render: () => <span className="text-sc-gray-400">—</span> },
  { key: "cpo", label: "CPO", align: "right", mono: true, render: () => <span className="text-sc-gray-400">—</span> },
